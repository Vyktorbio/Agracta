...
def _rodar_anova(relatorio, dados, rinfo, fatores_cols, fatores_vals,
    ...
    if res_anova["transformacao"]:
        decisao += (f"Pressupostos violados na escala original → aplicada "
                    ...
                    "ANOVA e pós-testes paramétricos usam a mesma escala "
    ...
    valores_modelo = np.asarray(res_anova["_resp_modelo"], float)
    medias_modelo = {}
    for grp in sorted(set(chaves)):
        grupo_modelo = valores_modelo[np.array([c == grp for c in chaves])]
        grupo_modelo = grupo_modelo[np.isfinite(grupo_modelo)]
        if grupo_modelo.size:
            medias_modelo[grp] = float(np.mean(grupo_modelo))
    comparacoes = {}
