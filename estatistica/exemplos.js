...
// Exemplos do módulo "Validação de método"
window.EXEMPLOS_VALIDACAO = {
  "Validação analítica — recuperação e precisão": {
    papeis: {
      resposta:"resultado",
      concentracao:"referencia",
    ...
    colunas: ["matriz","condicao","analista","dia","referencia","rep","branco","resultado"],
    linhas: (() => {
      const out = [];
      const niveis = [0, 1, 2, 5, 10, 20];
      const matrizes = ["Agua", "Extrato"];
      const analistas = ["A1", "A2"];
      const dias = ["D1", "D2"];
      const condicoes = ["Nominal", "pH +0,2"];
      let k = 0;
          ...
          analistas.forEach(analista => {
            dias.forEach(dia => {
              for(let rep=1; rep<=2; rep++){
                const condicao = condicoes[k % condicoes.length];
                const branco = nivel === 0 ? "sim" : "";
                const efeitoMatriz = matriz === "Extrato" ? 0.018 : -0.006;
                const efeitoAnalista = analista === "A2" ? 0.012 : -0.004;
                const efeitoDia = dia === "D2" ? -0.009 : 0.006;
                const efeitoCond = condicao === "pH +0,2" ? 0.01 : 0;
                const ruido = (((k * 17 + rep * 7) % 11) - 5) / 1000;
                const resposta = nivel === 0
                  ? 0.018 + Math.abs(ruido)
                  : nivel * (1 + efeitoMatriz + efeitoAnalista + efeitoDia + efeitoCond + ruido);
                out.push([matriz, condicao, analista, dia, nivel, rep, branco, Number(resposta.toFixed(4))]);
                k++;
              }
            });
          });
    ...
    obs: "Validação de método: linearidade, faixa, sensibilidade, LD/LQ, Grubbs, homocedasticidade, recuperação, precisão por analista/dia/matriz, robustez e Horwitz.",
  },
};

window.EXEMPLOS_FORENSE = {
  "Contagens suspeitas (subdispersão)": {
    papeis: { resposta: "resp", tratamento: "trat" },
    colunas: ["trat", "resp"],
    linhas: [
      ["T1", 6], ["T1", 7], ["T1", 6], ["T1", 7], ["T1", 6],
      ["T2", 8], ["T2", 9], ["T2", 10], ["T2", 9], ["T2", 8],
      ["T3", 11], ["T3", 14], ["T3", 12], ["T3", 11], ["T3", 14],
      ["T4", 15], ["T4", 14], ["T4", 16], ["T4", 15], ["T4", 14],
      ["T5", 5], ["T5", 6], ["T5", 4], ["T5", 5], ["T5", 5],
      ["T6", 9], ["T6", 11], ["T6", 8], ["T6", 9], ["T6", 10],
    ],
    obs: "Repetições apertadas demais para contagem de campo. Índice de dispersão deve marcar FLAG → veredito OBSERVAR (régua Conservador) ou INVESTIGAR (régua Sensível).",
  },
  "Contagens de campo (genuínas)": {
    papeis: { resposta: "resp", tratamento: "trat" },
    colunas: ["trat", "resp"],
    linhas: [
      ["T1", 2], ["T1", 9], ["T1", 4], ["T1", 13], ["T1", 6],
      ["T2", 11], ["T2", 3], ["T2", 17], ["T2", 8], ["T2", 14],
      ["T3", 7], ["T3", 18], ["T3", 2], ["T3", 11], ["T3", 9],
      ["T4", 1], ["T4", 8], ["T4", 14], ["T4", 4], ["T4", 12],
      ["T5", 6], ["T5", 16], ["T5", 3], ["T5", 9], ["T5", 21],
    ],
    obs: "Contagens com sobredispersão natural (variância > média), dígitos finais ~uniformes e extremos presentes. Esperado: SEM SINAIS RELEVANTES.",
  },
  "Acoplamento entre 2 avaliações": {
    papeis: { resposta: "aval1", tratamento: "trat", resposta2: "aval2" },
    colunas: ["trat", "aval1", "aval2"],
    linhas: [
      ["T1", 2, 3], ["T1", 9, 10], ["T1", 4, 5], ["T1", 13, 14], ["T1", 6, 7],
      ["T2", 11, 12], ["T2", 3, 4], ["T2", 17, 18], ["T2", 8, 9], ["T2", 14, 15],
      ["T3", 7, 8], ["T3", 18, 19], ["T3", 2, 3], ["T3", 11, 12], ["T3", 9, 10],
      ["T4", 1, 2], ["T4", 8, 9], ["T4", 14, 15], ["T4", 4, 5], ["T4", 12, 13],
    ],
    obs: "Duas avaliações que deveriam ser independentes casam repetição-a-repetição (aval2 = aval1 + 1). O teste de permutação deve marcar acoplamento como FLAG.",
  },
};
