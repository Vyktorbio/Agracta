...
function sections(c,d){const s=c.projection,st=c.study;
 ...
 {title:'Avaliações',text:'Preenchimento operacional no instante da exportação. Uma nota parcial não conclui a avaliação; finalizar o estudo preserva as lacunas registradas.',headers:['Avaliação','Data','Variáveis','Preenchimento','Valores completos','Observações'],rows:A(st.avaliacoes).map((a,i)=>{
   const id=a.id||'av-'+i,p=(A(s.avaliacoes).find(x=>x.id===id)||{}).progresso;
   return [id,a.data,A(a.variaveis).join(', '),p?(p.complete?'Completa':p.started?'Parcial':'Pendente'):'Não conferido',p?(p.total?p.filled+' de '+p.total:'Sem grade definida'):'Não conferido',a.obs||''];
 })},
