...
function showD(id){
  ...
  h+='<div style="margin-top:12px;display:flex;gap:16px;flex-wrap:wrap;font-size:11px;color:#5b616a;border-top:1px solid #e2e5e8;padding-top:10px">'+
     '<span title="Área calculada do polígono">&#128208; <b style="color:#15171a">'+(aHa!=null?aHa.toFixed(2)+' ha':'—')+'</b> <span style="color:#5b616a">medida</span></span>'+
     (dim?'<span title="Comprimento × largura — retângulo de menor área que encaixa na quadra (medido ao longo dos eixos dela)">&#128207; <b style="color:#15171a">'+Math.round(dim.comprimento)+' &times; '+Math.round(dim.largura)+' m</b> <span style="color:#5b616a">C&times;L</span></span>':'')+
     (ctr?'<span title="Coordenadas do centro da quadra">&#128205; <b style="color:#15171a">'+ctr[0].toFixed(5)+', '+ctr[1].toFixed(5)+'</b></span>':'')+
     '</div>';
...
function renderStudyEditModal(){
  ...
  h+='<div style="margin:0 0 14px;padding:14px;border:1px solid #cfe3d6;border-radius:18px;background:#eef7f1">'+
    '<div style="font-size:13px;font-weight:850;color:#17693a">Protocolo da planilha</div>'+
    '<div style="font-size:10px;color:#5a6b61;line-height:1.45;margin:3px 0 9px">Abra uma cópia do modelo ou copie a planilha inteira e cole abaixo. O Agracta preenche o que reconhecer; o restante continua manual.</div>'+
    '<div style="display:flex;gap:7px;flex-wrap:wrap"><button type="button" onclick="document.getElementById(\'protoFile\').click()" style="background:linear-gradient(180deg,#23975a,#17693a);color:#fff;border:0;border-radius:999px;padding:9px 14px;font-weight:800;cursor:pointer;box-shadow:0 4px 12px rgba(23,105,58,.25)">Abrir .xls/.xlsx</button>'+
    '<input id="protoFile" type="file" accept=".xls,.xlsx,.xlsm" onchange="importStudyProtocolFile(this)" style="display:none">'+
    '<button type="button" onclick="var x=document.getElementById(\'protoPasteBox\');x.style.display=x.style.display===\'none\'?\'block\':\'none\'" style="background:#fff;color:#33443a;border:1px solid rgba(22,32,26,.12);border-radius:999px;padding:9px 14px;font-weight:800;cursor:pointer">Copiar e colar</button></div>'+
    '<div id="protoPasteBox" style="display:none;margin-top:9px"><textarea id="protoPaste" rows="5" placeholder="No Excel/Sheets: selecione tudo a partir de A1, copie e cole aqui." style="width:100%;box-sizing:border-box;background:#fff;color:#26312b;border:1px solid #cfe3d6;border-radius:12px;padding:8px;font:11px/1.4 ui-monospace,Menlo,monospace"></textarea><button type="button" onclick="importStudyProtocolPaste()" style="width:100%;margin-top:6px;background:linear-gradient(180deg,#23975a,#17693a);color:#fff;border:0;border-radius:999px;padding:10px;font-weight:850;cursor:pointer;box-shadow:0 4px 12px rgba(23,105,58,.25)">Ler e preencher o estudo</button></div>'+
    (po.nome?'<div style="margin-top:9px;padding-top:8px;border-top:1px solid #d7e6db;font-size:10px;color:#5a6b61"><b style="color:#17352a">'+esc(po.nome)+'</b> · '+(pm.cultura?esc(pm.cultura):'cultura não informada')+(pm.cultivar?' · '+esc(pm.cultivar):'')+' · '+s.tratamentos.length+' tratamentos · '+s.avaliacoes.length+' avaliações<br><span style="color:#7d8b81">A quadra e o mapa não são alterados pela importação.</span></div>':'')+
    '</div>';
